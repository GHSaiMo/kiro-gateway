...
class KiroAccountSwitcher:
    ...
    @staticmethod
    def _is_usable_account(
        account: CockpitKiroAccount,
        *,
        ...
        Args:
            account: Cockpit account snapshot.
            require_fresh_access_token: Whether the account must have an access
                token outside the refresh window.
    ...
    async def switch_to_next_account(
        ...
        *,
        require_fresh_access_token: bool = True,
        reason: str = "monthly quota exhaustion",
    ) -> Optional[KiroAuthManager]:
        """
        Marks the unavailable account and rotates to the next usable account.

        Args:
            exhausted_account_id: Account id that should be excluded from the
                next selection.
            require_fresh_access_token: Whether fallback candidates must have a
                fresh access token.
            reason: Human-readable reason for logging the rotation.
        ...
        async with self._lock:
            ...
            if exhausted_id != current_id and refreshed_current is not None:
                if self._is_usable_account(
                    refreshed_current,
                    require_fresh_access_token=require_fresh_access_token,
                ):
            ...
            next_account = self._pool.pick_next_account(
                exclude_ids=candidates,
                require_fresh_access_token=require_fresh_access_token,
            )
            if next_account is None:
                logger.warning(
                    f"No alternate Cockpit account available after {reason}: "
            ...
            logger.info(
                f"Switched Kiro account after {reason}: "
